...
class IcpClassifier(BaseIcp, ClassifierMixin):
	...
	def predict(self, x, significance=None):
		...
		p2 = np.zeros((n_test_objects, self.classes.size+3))
		...
		for i in range(len(self.classes)):
			for j in range(n_test_objects):
                ...
                #to return n_nonconform removing smoothing from the p values calculation
				p2[j, i] = calc_p(ncal_ngt_neq[j, i, 0],
				                 ncal_ngt_neq[j, i, 1],
				                 ncal_ngt_neq[j, i, 2])

				p2[j, i+2] = ncal_ngt_neq[j, i, 1]+ncal_ngt_neq[j, i, 2]
				p2[j, 4] = ncal_ngt_neq[j, i, 0]                
