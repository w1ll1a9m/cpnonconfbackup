...
def class_one_err(prediction, y, significance=None):
    ...
    idx = list(filter(lambda x: np.sum(prediction[x, :]) == 1, idx))
    errors = list(filter(lambda x: not prediction[x, int(y[x])], idx))
    #print('idx: ', list(idx))
    #print('errors: ', list(errors))
    #print('len_idx: ', len(list(idx)))

    if len(idx) > 0:
        #print('size_errors: ',np.size(errors) )
        #print('size_list_errors: ',np.size(list(errors)) )
        #print('len_errors: ', len(list(errors)))
        #print('result: ', np.size(errors) / np.size(idx) )
        return np.size(errors) / np.size(idx)
    else:
        return 0

def class_multi_err(prediction, y, significance=None):
    """Calculates the error rate of conformal classifier predictions containing
...
def class_two_c(prediction, y, significance):
    """Calculates the rate of doubleton predictions (prediction sets containing
    only a single class label) of a conformal classification model.
    """
    prediction = prediction > significance
    n_doubletons = np.sum(1 for _ in filter(lambda x: np.sum(x) == 2,
                                            prediction))
    return n_doubletons / y.size


def class_multi_c(prediction, y, significance):
    """Calculates the rate of doubleton predictions (prediction sets containing
    only a single class label) of a conformal classification model.
    """
    prediction = prediction > significance
    n_multi = np.sum(1 for _ in filter(lambda x: np.sum(x) > 1,
                                            prediction))
    return n_multi / y.size
