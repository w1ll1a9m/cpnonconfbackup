...
class TcpClassifier(BaseEstimator, ClassifierMixin):
	...
	def __init__(self, nc_function, condition=None, smoothing=True, p_comp=False):
	...
	def predict(self, x, significance=None):
		...
		p2 = np.zeros((n_test, self.classes.size+3))
		for i in range(n_test):
			for j, y in enumerate(self.classes):
                ...
                #to compute p values old fashion or in the scoring function
                
				if self.p_comp == False:
					ngt = (scores[:-1] > scores[-1]).sum()
					neq = (scores[:-1] == scores[-1]).sum()
					n_train = scores[:-1].shape[0]
					self.smoothing = False                
					p[i, j] = calc_p(n_train, ngt, neq, self.smoothing)
                
                    #to return n_nonconform, removing smoothing from p values calculation
					p2[i, j] = calc_p(n_train, ngt, neq)
					p2[i, j+2] = ngt+neq
					p2[i, 4] = n_train
				else:
					p[i, j] = scores
